...
class PER_SAC(object):
    def __init__(self, num_inputs, action_space, args, device, per_alpha=0.7, min_priority=1): #0.4
        ...
        if self.policy_type == "Gaussian":
            ...
            self.actor_optimizer = Adam(self.actor.parameters(), lr=1e-4) #args.lr
        else:
            self.alpha = 0
    ...
    def update_parameters(self, replay_buffer, updates, batch_size=32):
          state_batch, action_batch, next_state_batch, reward_batch, mask_batch, ind, weight_batch = replay_buffer.sample(batch_size)
          with torch.no_grad():
          ...
          td_loss1 = (qf1 - qf1_next_target).abs()
          td_loss2 = (qf2 - qf2_next_target).abs()
